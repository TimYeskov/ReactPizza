  ...
  setActivePriceindex(index)
    ...
    React.useEffect(()=>{
       const fetchPizza= async ()=>{
        try {
            const{data}= await axios.get(`https://63ca304ef36cbbdfc7539138.mockapi.io/Pizzas/${id}`)
            setPizza(data)
        } catch (error) {
          ...
          className={activeSizeIndex===index ? 'active' : ""}>{size}</label>
            ...
            <div className='top__price'>
                ...
                <div className="item-count">
                 <button  onClick={()=>onCountMinus()}><img src="../img/minus.svg" alt="plus" className='minus'/></button>
                  <div>{item.count}</div>
                  <button><img src="../img/plus.svg" alt="minus"  className='plus' onClick={()=>setCount(count++)}/></button>
                </div>
            </div>
            </div>
            <div className='top__total-price'>
                ...
                <div className='total-price__button'>
                    ...
                    <button className='button__cart'><img src='../img/lala.svg' alt="cart" onClick={()=>onButtonCart()}/></button>
                </div>
            </div>
